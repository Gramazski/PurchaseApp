"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
/**
 * Created by Stanislau_Hramatskiu on 7/24/2017.
 */
var core_1 = require("@angular/core");
var logged_control_service_1 = require("../helpers/logged-control.service");
var router_1 = require("@angular/router");
var TopMenuComponent = (function () {
    function TopMenuComponent(loggedControlService, router) {
        this.loggedControlService = loggedControlService;
        this.router = router;
    }
    TopMenuComponent.prototype.logout = function () {
        this.loggedControlService.logout();
        this.router.navigate(['login']);
    };
    return TopMenuComponent;
}());
TopMenuComponent = __decorate([
    core_1.Component({
        selector: 'top-menu',
        templateUrl: './src/app/shared/menu/top-menu.component.html'
    }),
    __metadata("design:paramtypes", [logged_control_service_1.LoggedControlService, router_1.Router])
], TopMenuComponent);
exports.TopMenuComponent = TopMenuComponent;
//# sourceMappingURL=top-menu.component.js.map